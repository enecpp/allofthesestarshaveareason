...
public class FFmpegService : IFFmpegService
{
    ...
    public async Task<string> ExtractAudioAsync(string videoPath, string outputPath, CancellationToken cancellationToken = default)
    {
        if (!IsFFmpegAvailable())
        {
            throw new FileNotFoundException(
                "FFmpeg bulunamadý! FFmpeg'i indirip sistem PATH'ine ekleyin veya proje klasörüne koyun. " +
                "Ýndirme: https://ffmpeg.org/download.html");
        }
        ...
        {
            throw new InvalidOperationException($"Ses dosyasý oluþturulamadý: {outputPath}");
        }
        ...
        CancellationToken cancellationToken = default)
    {
        if (!IsFFmpegAvailable())
        {
            throw new FileNotFoundException("FFmpeg bulunamadý!");
        }
    ...
    private async Task ExecuteFFmpegAsync(string arguments, CancellationToken cancellationToken)
    {
        ...
        if (process.ExitCode != 0)
        {
            ...
            throw new InvalidOperationException($"FFmpeg baþarýsýz oldu (exit code {process.ExitCode})");
        }
    }
